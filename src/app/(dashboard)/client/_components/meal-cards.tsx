...
import { useMeals } from "../_services/use-meal-queries";
import { useDeleteMeal } from "../_services/use-meal-mutations";
...
const MealCards = () => {
  ...
  const calculateTotalCalories = (mealFoods: any) => {
    return mealFoods.reduce((total: any, mealFood: any) => {
  ...
  const calculateNutritionTotals = (meals: any) => {
    return (
      meals?.reduce(
        (totals: any, meal: any) => {
          meal.mealFood.forEach((mealFood: any) => {
